...
while [ $b -le 8000 ]; do
    ...
    printf "\n%05dRAV1EAV1|FFmpeg/ffmpeg|twopass|S|-pix_fmt|yuv420p|-f|mp4|-movflags|+faststart|-vcodec|librav1e|-speed|4|-b:v|%dk|-keyint_min|48|-g|48|-hide_banner|-nostats;\\" $b $b
    printf "\n%05dSVTAV1|FFmpeg/ffmpeg|vbr|F|-pix_fmt|yuv420p|-f|mp4|-movflags|+faststart|-vcodec|libsvt_av1|-preset|2|-b:v|%dk|-maxrate:v|%0.0fk|-bufsize:v|%0.0fk|-minrate:v|%dk|-rc|vbr|-sc_detection|0|-keyint_min|48|-g|48|-hide_banner|-nostats;\\" $b $b $(echo "$b * 1.5" | bc -l) $(echo "$b * 3" | bc -l) $b
